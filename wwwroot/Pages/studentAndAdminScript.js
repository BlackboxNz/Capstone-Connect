...
const getUsers = () => {
  ...
  if(level == "Admin"){
    ...
    getStudents(user-email);
  }
  else{
    getVisitors(user-email);
  }
}

const getAdmin = () => {
  const fetchPromise = fetch(`/webapi/GetAllAdmin`,
      {
          headers: {
              "Accept": "application/json",
              "Access-Control-Allow-Origin": "/webapi/GetAllAdmin"
          }
      }
  );const streamPromise = fetchPromise.then((response) => response.json());
  streamPromise.then((data) => populateUsers(data));
}
const getStudents = () => {
  const fetchPromise = fetch(`/webapi/GetStudents`,
      {
          headers: {
              "Accept": "application/json",
              "Access-Control-Allow-Origin": "/webapi/GetAllAdmin"
...
const getVisitors = () => {
  const fetchPromise = fetch(`/webapi/GetAllVisitors`,
      {
          headers: {
              "Accept": "application/json",
              "Access-Control-Allow-Origin": "/webapi/GetAllVisitors"
          }
      }
  );const streamPromise = fetchPromise.then((response) => response.json());
  streamPromise.then((data) => populateUsers(data));
